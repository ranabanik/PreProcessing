...
%   LibTIFF web site at <http://www.simplesystems.org/libtiff/>.
%
...
%   Copyright 2009-2018 The MathWorks, Inc.
    ...
    methods
        ...
        function obj = Tiff(filename,mode)
            ...
            validateattributes(mode,{'char','string'},{'nonempty'},'','MODE');
        ...
        %   either via the Tiff.TagID property or as a char or string. 
        %
        ...
        %   See also Tiff.setTag.
            ...
            switch(class(tagId))
                ...
                case 'string' 
                    validateattributes(tagId, {'string'}, {'scalar'}, mfilename);
                    tagId = convertStringsToChars(tagId);
                    tagValue = tifflib('getField',obj.FileID,Tiff.TagID.(tagId));

                case 'double'
                    validateattributes(tagId,{'double'},{'scalar'},mfilename,'TAGID');
                    tagValue = tifflib('getField',obj.FileID,tagId);
                    
                otherwise
                    % Invalid type, throw an error
                    validateattributes(tagId,{'char','string','double'},{'vector'},mfilename,'TAGID');
            end
        end
        ...
        %   See also Tiff.TagID, Tiff.getTag, Tiff.rewriteDirectory.
            ...
            switch(class(varargin{1}))
                case 'struct' 
                    validateattributes(varargin{1}, {'struct'}, {'scalar'}, mfilename);
                    tagstruct = varargin{1};
                    tagstruct = matlab.io.internal.imagesci.convertStringsInStruct(tagstruct);
                ...
                case 'string'
                    % The user gave a string for the tag.
                    validateattributes(varargin{1}, {'string'}, {'scalar'}, mfilename);
                ...
                otherwise
                    % Invalid type, throw an error
                    validateattributes(varargin{1}, {'struct','string','char','numeric'}, {'vector'},mfilename,'TAG');
            end
        end
    ...
    methods(Access='private')
        ...
        function setTagInFile(obj, varargin)
            try
                tifflib('setField',obj.FileID, ...
